...
import fr.flastar.programme.neededclasses.Average;
import fr.flastar.programme.neededclasses.TimeCalculator;
...
public class LogicManager implements Runnable {
    ...
    int LOOP;
    String inputFile = "inputfiles/players_init.txt";

    String outputFile = "outputfiles/calcultime_graph.txt";

    Average timeAverageCopy;
    Average timeAverageGame;
    Average timeAverageAdd;
    ...
    public LinkedHashMap<String, Integer[]> getPlayerMap() {
        return playerMap;
    }
    ...
    public ArrayList<Player> initProgram() throws Exception {
        ArrayList<Player> players = new ArrayList<>();
        File playerFile = new File(this.getInputFile());
        try (BufferedReader fileRead = new BufferedReader(new FileReader(playerFile))) {
            ...
            while ((line = fileRead.readLine()) != null) {  // lecture du fichier jusqu'à la fin
                ...
                else if (line.matches("^[a-zA-Z]+,[a-zA-Z0-9_]+((,[0-9.]+){3})?(,[0-9])?+$")) {
    ...
    public void run() {
        ...
        while (LOOP < 0) {
            try {
                String inputValue = JOptionPane.showInputDialog(JOptionPane.getRootFrame(),
                        "How much time execute ?", "Time to execute", JOptionPane.INFORMATION_MESSAGE);
                inputValue = inputValue.replaceAll("\\s", "");
                LOOP = Integer.parseInt(inputValue);
            } catch (Exception ex) {
                LOOP = -1;
            }
        }
        ...
        timeAverageCopy = new Average();
        timeAverageGame = new Average();
        timeAverageAdd = new Average();
        ...
        LinkedHashMap<String, Integer[]> playerWinTried = new LinkedHashMap<>();
        for (Map.Entry<String, Integer> entry : list) {
            playerWinTried.put(entry.getKey(), new Integer[]{entry.getValue(),
                    playerWinUntried.get(entry.getKey())[1]});
        }

        totalTime = calculTotalTime.getTimer();

        playerMap = playerWinTried;
    }

    public void writeGameInfos() throws Exception {
        try (FileOutputStream fos = new FileOutputStream(getOutputFile())) {
            fos.write(String.format("Total game : %,d, Calcul time : %,.2f s, Total kills : %,d\n",
                    LOOP, totalTime, LOOP * (players.size()-1)).getBytes());
